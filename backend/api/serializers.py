    ...
    def validate(self, attrs):
        ...
        if request and request.user and not self.instance:
            ...
            if book and Reservation.objects.filter(
                ...
                raise serializers.ValidationError(
                    {"non_field_errors": "You already have an active reservation for this book."}
                )
...
class UserProfileSerializer(serializers.ModelSerializer):
    ...
    def get_total_books_returned(self, obj):
        return obj.transactions.filter(status='returned').count()

    def get_current_fine(self, obj):
        unpaid_fines = obj.transactions.filter(fine_paid=False).aggregate(
